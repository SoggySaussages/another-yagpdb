...
//	}
	...
	if ic.Type == discordgo.InteractionMessageComponent {
		...
		tmplCtx := templates.NewContext(gs, cs, ms, fmt.Sprintf("%d,%s,%s,%d", 1, ic.MessageComponentData().CustomID, ic.Token, ic.Message.ID))
	...
	if ic.Type == discordgo.InteractionModalSubmit {
		...
		cs := gs.GetChannel(ic.ChannelID)
		ms := dstate.MemberStateFromMember(ic.Member)
		tmplCtx := templates.NewContext(gs, cs, ms, fmt.Sprintf("%d,%s,%s,%d,%s", 2, ic.ModalSubmitData().CustomID, ic.Token, ic.Message.ID, ic.ModalSubmitData().Components[0].(*discordgo.ActionsRow).Components[0].(*discordgo.TextInput).Value))
		logger.Debug("Executing custom command, standby...")
		ExecuteCustomCommand(cmd, tmplCtx, true)
		logger.Debug("Custom command executed")
		return
